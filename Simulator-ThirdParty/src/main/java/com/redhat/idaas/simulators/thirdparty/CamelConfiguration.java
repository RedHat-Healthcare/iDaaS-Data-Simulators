...
public class CamelConfiguration extends RouteBuilder {
  ...
  private String getFTPUri(String hostID, int port, String uID, String Pwd) {
    String portNumber = String.valueOf(port);
    String svrConnection = "netty4:tcp://"+ hostID +":" + portNumber + "?sync=true";
    return svrConnection;
  }

  private String getFTPUriDirectory(String dirName) {
  ...
  public void configure() throws Exception {
     ...
     *
     */
        from(getFTPUriDirectory(config.getText_Directory()))
            // Auditing
            .routeId("ftpFileSimulator")
            .routeDescription("ftpFileSimulator")
            .convertBodyTo(String.class)
            .setProperty("processingtype").constant("ftp-sim")
            ...
            .to(getFTPUri(config.getFtpHost(),config.getFtpPort(), config.getFTPUserName(), config.getFTPPassword())
        ;
